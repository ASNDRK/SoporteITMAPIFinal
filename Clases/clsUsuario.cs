...
namespace SoporteITMAPI.Clases
{
    public class clsUsuario
    {
        ...
        public string Actualizar(int Perfil)
        {
            try
            {
                ...
                //if (cifrar.CifrarClave())
                {
                    ...
                    dbSoporte.Usuario_Perfil.AddOrUpdate(usuarioPerfil);
                    dbSoporte.SaveChanges();
        ...
        {
            return dbSoporte.Usuarios.FirstOrDefault(U => U.Documento_Empleado == Documento);
        }
        public Usuario_Perfil ConsultarUsuarioPerfil(int idUsuario)
        {
            return dbSoporte.Usuario_Perfil.FirstOrDefault(UP => UP.idUsuario == idUsuario);
        }
        public string Insertar(int Perfil)
        {
            try
            {
                ...
                if (cifrar.CifrarClave())
                {
                    ...
                    usuarioPerfil.idPerfil = Perfil;
                    usuarioPerfil.Activo = true;
                    dbSoporte.Usuario_Perfil.Add(usuarioPerfil);
                    dbSoporte.SaveChanges();
        ...
        public string Activar(int idUsuarioPerfil, bool Activo)
        {
            try
            {
                Usuario_Perfil usuario_Perfil = dbSoporte.Usuario_Perfil.FirstOrDefault(u => u.id == idUsuarioPerfil);
